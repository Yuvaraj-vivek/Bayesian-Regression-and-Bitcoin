...
def computeDelta(wt, X, Xi):
    ...
    for i in range(0,50):
    	mean_Xi = np.mean(Xi.iloc[i][:M])
    	numerator=0.00
    	for j in range(0,M):
    		numerator+= (X.iloc[j]-mean_X)*(Xi.iloc[i][j]-mean_Xi)
    	denominator = M * np.std(X[:M]) * np.std(Xi.iloc[i][:M])
    	similiarity = float(numerator/denominator)
    	similiarity_array.append(similiarity)
    	
    expectation_numerator= 0
    expectation_denominator = 0 
    for i in range(0,50):
    	expectation_numerator += Xi.iloc[i][M]*(np.exp(wt*(similiarity_array[i])))
    	expectation_denominator += (np.exp(wt*(similiarity_array[i])))
    value = float(expectation_numerator/expectation_denominator)
    return value
